...
(function ($) {
        ...
        init: function (options, swfUploadOptions) {
            ...
            return this.each(function () {
                ...
                var settings = $.extend({
                    ...
                    showError: null,           // The function to use to display error messages
                ...
                if (flashInstalled) {
                    ...
                    swfuploadify.queueData = {
                        ...
                        errorMsg: 'بعض الملفات لم يتم إضافتها:'
                    };
                ...
                if (args[0]) {
                    ...
                    if (args[0] == '*') {
                        ...
                        $('#' + settings.queueID).find('.uploadify-queue-item').each(function () {
                            ...
                            $(this).find('.data').removeClass('data').html(' - تم الإلغاء');
                        ...
                        for (var n = 0; n < args.length; n++) {
                            ...
                            $('#' + args[n]).find('.data').removeClass('data').html(' - تم الإلغاء');
                    ...
                    $item.find('.data').removeClass('data').html(' - تم الإلغاء');
                    $item.find('.uploadify-progress-bar').remove();
                    $item.delay(1000).fadeOut(500, function () {
                        $(this).remove();
                    });
                }
                handlers.onCancel(fileID, settings);
            });
    ...
    var handlers = {
        ...
        onDialogOpen: function () {
            ...
            this.queueData.errorMsg = 'بعض الملفات لم يتم إضافتها:';
            ...
            if ($.inArray('onDialogClose', settings.overrideEvents) < 0) {
                if (this.queueData.filesErrored > 0) {
                    if (settings.showError != null)
                        settings.showError(this.queueData.errorMsg);
                    else
        ...
        onSelect: function (file) {
            ...
            for (var n in this.queueData.files) {
                ...
                if (queuedFile.uploaded != true && queuedFile.name == file.name) {
                    var replaceQueueItem = confirm('الملف "' + file.name + '" موجود بالفعل في الملفات قيد التحميل.\nهل ترغب في إستبدال الملف الموجود ضمن الملفات قيد التحميل؟');
                    if (!replaceQueueItem) {
            ...
            if (fileSize == 0 && file.size != 0) {
                // If a file is < 1024 bytes, round it up to 1 KB
                fileSize = 1;
            }
        ...
        onCancel: function (fileID, settings) {
            // Call the user-defined event handler
            if (settings.onCancel) settings.onCancel.apply(this, arguments);
        },
                ...
                switch (errorCode) {
                    case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
                        if (settings.queueSizeLimit > errorMsg) {
                            this.queueData.errorMsg += '\nعدد الملفات المختارة يتجاوز الحد المتبقي المسموح به للتحميل (' + errorMsg + ').';
                        } else {
                            this.queueData.errorMsg += '\nعدد الملفات يتجاوز الحد المسموح به للتحميل (' + settings.queueSizeLimit + ').';
                        }
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\nالملف "' + file.name + '" تتجاوز مساحته الحد المسموح به (' + settings.fileSizeLimit + ').';
                        break;
                    case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
                        this.queueData.errorMsg += '\nالملف "' + file.name + '" فارغ.';
                        break;
                    case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
                        this.queueData.errorMsg += '\nغير مسموح بتحميل الملف  "' + file.name + '" يجب تحميل ملف من نوع (' + settings.fileTypeDesc + ').';
                        break;
                }
            }
        ...
        onUploadError: function (file, errorCode, errorMsg) {
            ...
            switch (errorCode) {
                case SWFUpload.UPLOAD_ERROR.HTTP_ERROR:
                    errorString = 'خطأ في عملية التحميل (' + errorMsg + ')';
                    break;
                case SWFUpload.UPLOAD_ERROR.MISSING_UPLOAD_URL:
                    errorString = 'رابط التحميل ناقص';
                    break;
                case SWFUpload.UPLOAD_ERROR.IO_ERROR:
                    errorString = 'خطأ في قراءة الملف';
                    break;
                case SWFUpload.UPLOAD_ERROR.SECURITY_ERROR:
                    errorString = 'خطأ في نظام الأمان';
                    break;
                case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
                    if (settings.showError != null) {
                        settings.showError(errorMsg);
                    }
                    else
                        alert('تم الوصول إلي حد التحميل المسموح به (' + errorMsg + ').');
                    errorString = 'تم تخطي حد التحميل المسموح به';
                    break;
                case SWFUpload.UPLOAD_ERROR.UPLOAD_FAILED:
                    errorString = 'فشل في عملية التحميل';
                    break;
                case SWFUpload.UPLOAD_ERROR.SPECIFIED_FILE_ID_NOT_FOUND:
                    break;
                case SWFUpload.UPLOAD_ERROR.FILE_VALIDATION_FAILED:
                    errorString = 'خطأ في مصادقة الملف';
                    break;
                case SWFUpload.UPLOAD_ERROR.FILE_CANCELLED:
                    errorString = 'تم الإلغاء';
                    this.queueData.queueSize -= file.size;
            ...
            if (settings.checkExisting) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data == 1) {
                            var overwrite = confirm('الملف "' + file.name + '" موجود بالفعل على الخادم.\nهل ترغب في إستبدال الملف القديم؟');
                            if (!overwrite) {
                                this.cancelUpload(file.id);
                                $('#' + file.id).remove();
        ...
        onUploadSuccess: function (file, data, response) {
            ...
            if ($.inArray('onUploadSuccess', settings.overrideEvents) < 0) {
                $('#' + file.id).find('.data').html(' - تم التحميل');
            }
    ...
    $.fn.uploadify = function (method) {
        if (methods[method]) {
            ...
            $.error('الوظيفة ' + method + ' لا توجد في $.uploadify');
        }
