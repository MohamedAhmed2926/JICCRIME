...
namespace JIC.Crime.Repositories.Repositories
{
    public class DefendantsRepository : EntityRepositoryBase<Cases_CaseDefendants>, IDefendantsRepository
    {
        ...
        public void DeleteDefendant(int CaseID,long DefendantID)
        {
            ...
            {
                var DefendantObj = GetAllQuery().Where(x => x.CaseID == CaseID ).ToList(); //GetByID(VictimID );

                foreach (var z in DefendantObj)
                {
                    Delete(z);
                    Save();
                }
        ...
        public vw_CaseDefectsData GetDefendant(int caseID, long partyID)
        {
            return GetAllQuery().Where(defendant => defendant.CaseID == caseID && defendant.ID == partyID)
                .Select(defendant => new vw_CaseDefectsData
                {
                    ID = defendant.ID,
                    CaseID = caseID,
                    Crimes = defendant.Cases_DefendantsCharges.Select(charge =>new vw_KeyValue { ID = charge.Configurations_Lookups.ID, Name = charge.Configurations_Lookups.Name }).ToList(),
        ...
        public List<vw_CaseDefectsData> GetDefendantsByCaseID(int CaseID)
        {
            var defendants = (from _case in DataContext.Cases_Cases
                              ...
                              where _case.ID == CaseID && _defendant.IsActive && _case.IsDeleted != true
                              select new vw_CaseDefectsData
                              {
        ...
        {
           var result = (from _case in DataContext.Cases_Cases
            ...
            if (result.FirstOrDefault().OrderOfAssignmentNotComplete==true)
            {
                return CaseStatus.GoInFlow;

            }
            else
            {
                return CaseStatus.NotGoInFlow;
            }
            
        }
        public void ReOrderDefendantOrders(int caseID)
        {
            var Defendants = GetAllQuery().Where(defendant => defendant.CaseID == caseID).OrderBy(defendant => defendant.Order).ToList();
            foreach (var Defendant in Defendants)
            {
                Defendant.Order = Defendants.IndexOf(Defendant) + 1;
                Update(Defendant);
                Save();
            }
